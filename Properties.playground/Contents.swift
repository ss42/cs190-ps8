...
struct LocationTrack {
    ...
    var length: CLLocationDistance {
        ...
        {
            distance = locations.first!.distanceFromLocation(locations.first!)
            return distance // right now it just returns 1000 (in meters, which is one kilometer).

        }
        else{
           // for location in locations.count as! [CLLocation]
            for (var i = 0; i < locations.count-1; i += 1)
            {
                let fromLocation = locations[i]
                let toLocation = locations[i + 1]
...
class LocationTrackTestSuite: XCTestCase {
    ...
    func testLengthOfTrackWithTwoPoints() {
        let sanfrancisco = CLLocation(latitude: 37.774929, longitude: -122.419416)
        let oakland = CLLocation(latitude: 37.8044, longitude: -122.2711)
        let moraga = CLLocation(latitude: 37.841347, longitude: -122.110135)
        let twoPointTrack = LocationTrack(locations: [sanfrancisco, oakland, moraga])
        let expectedResult: CLLocationDistance = 28.22065585368208
        XCTAssertEqual(expectedResult, twoPointTrack.length, "Locations between two points.")
    }
}
/*:
